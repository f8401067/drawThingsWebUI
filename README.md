...
- 图片生成：支持自定义提示词、尺寸、seed、步数等参数 (图片历史界面可点击提示词或者seed进行一键复制,方便下次使用)
- 支持AI提示词润色(需要配置大模型API)
- 本地存储：自动保存用户输入，下次打开页面时自动填充
- 图片查看：支持放大、缩小、重置和保存生成的图片
- 耗时统计：记录每次生成的耗时并计算平均耗时
- **浏览器图片缓存**：基于 Service Worker 的智能缓存，第二次访问图片秒开，支持离线浏览
- **自动NSFW检测**：基于LLM的内容安全检测，自动识别并标记不当内容(支持点击右上角✅图标手动标记)
- **星级评分系统**：为每张图片提供1-5星及Bad评级功能支持支持
...
- **默认显示全部**：历史页面默认不设置日期筛选，充分利用缓存显示所有图片

详细说明请查看 [docs/IMAGE_CACHE_FEATURE.md](docs/IMAGE_CACHE_FEATURE.md)
...
│   ├── IMAGE_CACHE_FEATURE.md      # 图片缓存功能说明
│   └── *.md                    # 其他功能说明文档
│
├── scripts/               # 工具脚本
...
- 更丰富的图片编辑功能
- 团队协作功能增强
- 性能优化和缓存机制（已实现 Service Worker 图片缓存）
